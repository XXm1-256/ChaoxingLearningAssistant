...
默认入口：`BUILD_V1_42.bat`。直接构建并启动可运行 `START.bat`；完整便携包/安装器使用 `BUILD_FULL.bat`。
...
`START.bat` 会检查 `artifacts\QUICK_RUN_READY.txt` 中的 `Version: 1.42.0`，版本不匹配时自动重新构建。

Windows 编译与测试记录位于 `docs/validation/v1.42/`。真实学习通课程仍需实机账号环境验收。
