...
SCREENSHOT = ROOT / "docs" / "validation" / "v1.44" / "app-main.png"
ACCENT = "3D9B67"
PALE = "E8F5EC"
DARK = "17392D"
GRID = "D9E4DC"
...
set_font(subtitle.add_run("适用于 v1.46 Windows 便携版"), size=11.5, color="4E6C5C")
...
buttons = [
    ...
    ("静音 / 已静音", "点击静音后视频继续播放；切换视频仍保持静音。再次点击“已静音”恢复声音，不影响其他软件。"),
    ("课程列表", "单击课程即可进入；进入后还需要先在网页中手动点开任意一个视频。"),
...
add_body(doc, "学习通切换下一视频通常需要约半分钟。显示“下一视频加载中”时请保持页面开启，不要反复点击；视频时间开始增长后，程序才会确认播放成功。各加载或播放确认阶段最多等待 90 秒，成功后立即继续；超时仍未成功会停在当前目标并提示。")
...
add_steps(doc, [
    ...
    "进入课程后，必须先在网页中手动点开任意一个视频，并等待视频页面出现。",
    "再点一次“开始”，观察右侧的“当前视频”和“下一视频”。",
    "仍然无法继续时，打开“日志与反馈”查看记录。",
    "点“导出反馈包”，并说明课程页面、发生步骤和当时的视频位置。",
    "v1.46 会自动遮盖常见令牌、签名和 Cookie；公开发送前仍请检查并删除不便公开的课程名称、学校名称等内容。",
])
...
doc.core_properties.subject = "学习通课程视频播放助手 v1.46 使用说明"
