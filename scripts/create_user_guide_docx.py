...
set_font(subtitle.add_run("适用于 v1.41 Windows 便携版"), size=11.5, color="4E6C5C")
...
add_steps(doc, [
    ...
    "完成登录后，单击左侧课程。程序会自动切到章节页签并读取目录。",
    "点顶部的“开始”。若目录尚未生成，程序会先打开首个真实章节完成加载，再继续查找。",
...
buttons = [
    ...
    ("打开未完成章节", "无需先手动点章节；程序会在需要时加载目录，连续核对状态后再打开真实待播目标。"),
...
add_body(doc, "如果下一章节的视频需要进入后才加载，程序会先打开首个真实章节初始化目录，并在连续三次读到相同状态后再选目标；播放中会先显示待核验的下一章节，进入后再定位其中第一条真实未完成视频。")
...
add_steps(doc, [
    ...
    "点“打开未完成章节”后留意按钮和顶部提示；尚未手动打开过章节也没关系，程序会自动初始化并连续核对目录。",
...
doc.core_properties.subject = "学习通课程视频播放助手 v1.41 使用说明"
