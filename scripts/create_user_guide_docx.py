...
SCREENSHOT = ROOT / "docs" / "validation" / "v1.41" / "app-main.png"
ACCENT = "3D9B67"
PALE = "E8F5EC"
DARK = "17392D"
GRID = "D9E4DC"
...
set_font(subtitle.add_run("适用于 v1.43 Windows 便携版"), size=11.5, color="4E6C5C")
...
add_steps(doc, [
    ...
    "完成登录并进入课程后，必须先在网页中手动点开任意一个视频。",
    "视频页面出现后，再点“打开未完成章节”或顶部的“开始”。",
...
buttons = [
    ...
    ("打开未完成章节", "使用前必须先手动点开任意一个视频；页面加载完成后再查找真实待播目标。"),
...
add_body(doc, "先手动点开任意一个视频后，程序会连续核对目录状态再选择目标；播放中会先显示待核验的下一章节，进入后再定位其中第一条真实未完成视频。")
...
add_steps(doc, [
    ...
    "再点一次“开始”或“打开未完成章节”，观察右侧的“当前视频”和“下一视频”。",
    "仍然无法继续时，打开“日志与反馈”查看记录。",
    "点“导出反馈包”，并说明课程页面、发生步骤和当时的视频位置。",
    "v1.43 会自动遮盖常见令牌、签名和 Cookie；公开发送前仍请检查并删除不便公开的课程名称、学校名称等内容。",
])
...
doc.core_properties.subject = "学习通课程视频播放助手 v1.43 使用说明"
