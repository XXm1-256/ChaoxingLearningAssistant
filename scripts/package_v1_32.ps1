...
$Version = 'v1.32'
$FolderName = 'ChaoxingLearningAssistant_v1.32'
$ManifestPath = Join-Path $Root 'SOURCE_SHA256.json'
$MasterPath = Join-Path $Root 'ChaoxingLearningAssistant_MASTER_v1.32.txt'
$PayloadZip = Join-Path $OutputDirectory 'ChaoxingLearningAssistant_v1.32_master_payload.zip'
$SourceZip = Join-Path $OutputDirectory 'ChaoxingLearningAssistant_v1.32_Developer.zip'
$ReferenceZip = Join-Path $OutputDirectory 'ChaoxingLearningAssistant_v1.31_source.zip'
...
$master = @"
学习通课程播放辅助 — 累计式自包含 MASTER v1.32
日期：2026-09-11。当前开发基线为 v1.32，历史测试结论不得套用到本版。

本文件不再嵌套已损坏的旧 MASTER 字符串，而是从当前可读档案重新汇总全部版本历史。
只保留本 TXT 也能恢复源码：将下方 Python 代码保存为 restore_master.py，运行：
python restore_master.py ChaoxingLearningAssistant_MASTER_v1.32.txt restored_v1.32
...
if (Test-Path -LiteralPath $ReferenceZip) {
    ...
    try {
        [System.IO.Compression.ZipFileExtensions]::CreateEntryFromFile(
            $archive,
            $ReferenceZip,
            'reference/ChaoxingLearningAssistant_v1.31_source.zip',
            [System.IO.Compression.CompressionLevel]::Optimal) | Out-Null
    }
    finally {
        $archive.Dispose()
...
Write-Host "DEVELOPER_PACKAGE=$SourceZip"
Write-Host "MASTER_IN_DEVELOPER_PACKAGE=$MasterPath"
