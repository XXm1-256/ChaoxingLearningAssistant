param(
    [string]$OutputPath = (Join-Path (Split-Path -Parent $PSScriptRoot) 'docs\validation\v1.40\app-main.png')
)
...
$process = Get-Process -Name 'ChaoxingLearningAssistant' -ErrorAction Stop |
    ...
    Sort-Object @{ Expression = { $_.MainWindowTitle -like '*v1.40*' }; Descending = $true },
