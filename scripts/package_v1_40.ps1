...
$Version = 'v1.40'
$FolderName = 'ChaoxingLearningAssistant_v1.40'
$ManifestPath = Join-Path $Root 'SOURCE_SHA256.json'
$MasterPath = Join-Path $Root 'ChaoxingLearningAssistant_MASTER_v1.40.txt'
$PayloadZip = Join-Path $OutputDirectory 'ChaoxingLearningAssistant_v1.40_master_payload.zip'
$SourceZip = Join-Path $OutputDirectory 'ChaoxingLearningAssistant_v1.40_Developer.zip'
...
function Get-PayloadFiles {
    Get-ChildItem -LiteralPath $Root -File -Recurse | Where-Object {
        ...
        $relative -notmatch '(^|/)master-restored[^/]*/' -and
        $relative -notmatch '(^|/)guide-render(/|$)' -and
...
$master = @"
学习通课程视频播放助手 - 累计式自包含 MASTER v1.40
日期：2026-09-12。当前开发基线为 v1.40，历史测试结论不得套用到本版。

本文件不再嵌套已损坏的旧 MASTER 字符串，而是从当前可读档案重新汇总全部版本历史。
只保留本 TXT 也能恢复源码：将下方 Python 代码保存为 restore_master.py，运行：
python restore_master.py ChaoxingLearningAssistant_MASTER_v1.40.txt restored_v1.40
...
===== V1.40 SOURCE ZIP BASE64 BEGIN =====
$payloadBase64
===== V1.40 SOURCE ZIP BASE64 END =====
"@
