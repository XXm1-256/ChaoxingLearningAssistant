...
- 修正加载超时继续跳过章节的路径；有视频或状态未知时停留原处。
- 新播放器加载期间保留目标，实际播放进度前进后确认成功。
- 播放器等待延长到约 40 秒，覆盖真实课程中约半分钟的加载。
- 修正加载中 0/0 被旧视频结束记录影响，以及聚焦后误用旧播放器快照的问题。

## v1.44 - 2026-09-12

- 删除学生界面的“打开未完成章节”按钮及其专用事件入口。
- “开始”只接管学习通网页中由同学手动选择的当前视频，不再主动搜索其他未完成章节。
- 当前视频自然结束后的同章下一视频、跨章节续播和测验排除逻辑保持不变。
- 使用教学、常见问题、用户包、开发者包与累计 MASTER 升级到 v1.44。

## v1.43 - 2026-09-12

- 播放与重试全程锁定最初目标播放器，拒绝相同来源的旧 iframe 和 DOM 换位造成的目标漂移。
- 统一已加载视频与懒加载任务点的页面顺序；重复 mediaId 不再跨不同 iframe 合并。
- 区分“没有下一视频”和“下一视频切换失败”，失败时停住且不把整章记为已核验。
- 跨章节、手动切换、目录稳定和视频任务稳定使用一致的就绪门槛。
- 增加操作代次、持续人工干预检测、日志/反馈包脱敏、完整构建版本哈希门槛和 MASTER 恢复防护。
- 修正开发接手基线，并升级用户包、开发者包、教学、常见问题和累计 MASTER 到 v1.43。

## v1.42 - 2026-09-12

- 使用教学改为要求进入课程后先手动点开任意一个视频，再使用“打开未完成章节”或“开始”。
- 修复同章第二个任务点只显示 `第 2/2 个视频` 和 `00:00 / 00:00`，却没有真正加载播放的问题。
- 自动播放改为定向操作新播放器；空播放器不会提前确认成功，并会重新激活目标任务点。
- 后续章节状态尚未加载时按课程顺序显示为待核验章节，明确完成章节和非视频任务继续排除。
- 版本、教学、常见问题、用户包、开发者包与累计 MASTER 升级到 v1.42。

## v1.41 - 2026-09-12

- 修复课程刚进入、尚未手动点过章节时，“打开未完成章节”在空目录上直接误报没有的问题。
- 空目录时自动打开首个真实章节以触发完整目录加载，并排除返回课程、测验、作业、签到和考试入口。
- 查找目标前连续三次核对完整章节与视频状态；状态发生变化就重新计数，避免第一次选错、重复点击后才选对。
- “开始”、按钮查找及导航后的继续流程共用稳定目录读取逻辑。
- 版本、构建入口、教学、常见问题、用户包、开发者包和累计 MASTER 升级到 v1.41。

## v1.39 - 2026-09-12

- 修复当前章最后一个视频播放时，后续“明确未完成但目录未写明视频类型”的章节被排除，导致下一视频为空和“打开未完成章节”误报的问题。
- 跨章节候选改为两阶段锁定：播放中预告下一未完成章节，进入后再定位第一条真实未完成视频；章节若只剩测验或已完成视频则继续跳过。
- 新增两个 C# 回归用例，分别锁定待核验章节选择和播放中下一章节预告；升级构建、用户包、开发者包和累计 MASTER 到 v1.39 / 1.39.0。

## v1.38 - 2026-09-12

- 课程列表由双击改为单击进入，并立即切换到“章节”页签显示读取状态。
- 课程首页和课程工作页也进入章节目录刷新范围，不再只在已经打开具体章节后刷新。
- 章节目录初次为空时进行短暂重试；若页面稍后才创建目录 iframe，后台轮询会继续补读。
- 修复因此导致“打开未完成章节”在空目录上误报“没有可打开的视频”的问题。
- 新增课程工作页地址分类测试，并升级构建、用户包、开发者包和累计 MASTER 到 v1.38 / 1.38.0。

## v1.37 - 2026-09-12

- 修复章节总状态未完成、但章节内真实视频已全部完成时仍停留或重新播放的问题。
- 进入候选章节后再次读取视频任务；若视频均已完成，继续寻找后续含未完成视频的章节。
- “打开未完成章节”和视频自然结束后的顺序跳转共用同一套复核逻辑，避免两条路径结果不一致。
- 没有任务明细时，仅把明确标记为视频且未完成的章节作为待播候选，不再用普通章节总状态代替视频状态。
- 新增“两条视频已完成但章节因测验未完成”的回归测试，并升级构建、用户包、开发者包和累计 MASTER 到 v1.37 / 1.37.0。
...
- 修复学习通目录隐藏状态模板覆盖可见完成状态，导致已完成章节或视频被误选的问题。
- 明确使用 `jobUnfinishCount` 数量：0 表示已完成，大于 0 表示未完成。
- 目录只有未知状态时，“开始”“打开未完成章节”和自然结束后的全课程复核会等待并自动重扫。
- 用户包最外层新增《常见问题与处理方法》Markdown、DOCX、PDF，并同步更新使用教学。
