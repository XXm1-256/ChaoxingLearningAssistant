# ERROR_ARCHIVE - 当前版本 v1.37

## v1.37 章节未完成但视频已完成时误停 2026 09 12

- 宝贝反馈：点击“打开未完成章节”确实会切章，但目标章节的视频其实已看完；同样担心自然续播也会按章节总状态误入这类章节。
- 原因：学习通的章节总状态可能同时计算视频、测验和作业。原流程进入候选章节后只确认播放器出现，没有再核对该章真实视频是否全部完成，因此测验未完成也可能让已看完视频被当成目标。
- 修复：候选章节打开后重新扫描真实视频任务；若所有视频都有完成证据，则记录该章视频已核验并继续寻找后续章节。无任务明细时，只信任明确的视频类型，不再把普通章节未完成直接当成视频未完成。
- Debug 原理：把“章节状态”和“视频状态”分开判断，并在真实页面切换完成后做第二次核验。“打开未完成章节”和自然结束顺序跳转都经过同一准备函数，因此同一修复同时覆盖两条路径。
- 防复发：新增“两条真实视频均已完成、章节仍因测验未完成”的测试；空任务列表不能被误判为“视频都完成”，避免页面尚未加载时被过早跳过。
